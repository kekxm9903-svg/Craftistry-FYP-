...
class Order extends Model
{
    ...
    protected $fillable = [
        'user_id', 'type', 'total', 'stripe_session_id', 'payment_status',
        'payment_method', 'artist_id', 'title', 'description', 'price',
    ...
    public function scopeForArtist($query, ?int $artistId) { return $query->where('artist_id', $artistId); }
    public function scopeForBuyer($query, int $userId)     { return $query->where('user_id', $userId); }
    ...
    public function isRefundable(): bool
    {
        ...
        if (($this->refund_status ?? 'none') !== 'none') return false;
        if (!in_array($this->status, ['completed','shipped','preparing','processing'])) return false;
    ...
    public function getStatusChipClass(): string { return 'chip-' . $this->status; }
}