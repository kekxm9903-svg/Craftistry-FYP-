...
<div class="orders-inner">
    ...
    @php
        ...
        $activePills = $tabStatusPills[$currentCat] ?? $tabStatusPills[''];
    @endphp
    ...
    <div class="status-pills-card">
        <div class="status-row">
            @foreach($activePills as $val => $label)
                @if($val === '__refund__')
                    @php
        ...
        <div class="orders-list">
            ...
            @php
                ...
                $orderReview        = $order->has_review
                    ...
                    : null;
                $canEditReview      = $orderReview && $orderReview->created_at->diffInDays(now()) <= 30;
                ...
                <div class="oc-header">
                    ...
                    <div class="oc-header-right">
                        ...
                        @if($refundStatus === 'requested')
                            <span class="refund-inline-badge pending"><i class="fas fa-clock"></i> Refund Pending</span>
                        @elseif($refundStatus === 'refunded')
                            <span class="refund-inline-badge refunded"><i class="fas fa-check-circle"></i> Refunded</span>
                        @elseif($refundStatus === 'rejected')
                            <span class="refund-inline-badge rejected"><i class="fas fa-times-circle"></i> Refund Rejected</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="oc-body">
                    ...
                    @endphp
                        ...
                        <div class="oc-item-info">
                            <p class="oc-item-title">{{ $order->items->first()?->name ?? $order->notes ?? 'Artwork Order' }}</p>
                            ...
                            @if($firstItem)
                                <p class="oc-item-meta">
                                    ...
                                    @if($firstItem->variant ?? null) &middot; {{ $firstItem->variant }} @endif
                                </p>
                            @endif
                        </div>
                    ...
                    <div class="track-steps">
                        @foreach($trackingSteps as $i => $step)
                            ...
                            <div class="track-step {{ $isDone ? 'step-done' : '' }} {{ $isCurrent ? 'step-current' : '' }}">
                                <div class="track-dot">
                                    @if($isDone && !$isCurrent)<i class="fas fa-check" style="font-size:8px;"></i>@endif
                                </div>
                ...
                <div class="oc-footer">
                    ...
                    <div class="oc-actions">
                        ...
                        @if($canDownloadReceipt)
                            <a href="{{ route('orders.receipt', $order->id) }}" class="btn-craft btn-craft-receipt">
                        ...
                        @if($order->status === 'completed')
                            @if($order->has_review)
                                ...
                                @if($canEditReview)
                                    <a href="{{ route('reviews.edit', $orderReview->id) }}" class="btn-craft btn-craft-edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                @endif
                            @else
                                <a href="{{ route('reviews.create', $order->id) }}" class="btn-craft btn-craft-primary">
                                    <i class="fas fa-star"></i> Rate
                                </a>
                            @endif
                        @elseif($order->tracking_number && $order->getTrackingUrl())
                            <a href="{{ $order->getTrackingUrl() }}" target="_blank" rel="noopener" class="btn-craft btn-craft-track">
                        ...
                        @endif
                        @if($canCancel)
                            <button type="button" class="btn-craft btn-craft-cancel"
...
{{-- Cancel Order Modal --}}
<div id="cancelConfirmModal" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center;">
    <div style="position:absolute; inset:0; background:rgba(0,0,0,.48); backdrop-filter:blur(3px);" onclick="closeCancelConfirm()"></div>
...
{{-- Refund Modal --}}
<div id="refundModal" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center;">
    <div style="position:absolute; inset:0; background:rgba(0,0,0,.48); backdrop-filter:blur(3px);" onclick="closeRefundModal()"></div>
    <div id="refundModalInner"
        ...
        <div style="text-align:center; margin-bottom:20px;">
            <div style="width:56px; height:56px; background:linear-gradient(135deg,#fff8f0,#fef3c7);
                        border-radius:50%; display:flex; align-items:center; justify-content:center;
                        margin:0 auto 14px; border:2px solid #fcd34d; box-shadow:0 4px 12px rgba(252,211,77,.25);">
        ...
        <div id="refundInfoBox"
             style="background:#eff6ff; border:1px solid #bfdbfe; border-radius:9px;
                    padding:11px 14px; margin-bottom:16px; display:flex; align-items:flex-start; gap:9px;">
            ...
            <div style="display:flex; gap:10px;">
                <button type="button" onclick="closeRefundModal()"
                    style="flex:1; padding:11px; border-radius:8px; border:1.5px solid #e2e8f0;
                           background:#fff; color:#6b7280; font-size:0.88rem; font-weight:600; cursor:pointer; font-family:inherit;"
...
window.closeCancelConfirm = function () { document.getElementById('cancelConfirmModal').style.display = 'none'; };
window.executeCancelOrder = function () { document.getElementById('cancelOrderForm').submit(); };
